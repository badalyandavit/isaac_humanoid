...
class IsaacLabPPOConfig:
    ...
    experiment_name: str = "humanoid_direct_fair_ppo"
...
@dataclass
class IsaacLabPPOPopulationConfig:
    isaaclab_dir: str = "/workspace/IsaacLab"
    task: str = "Isaac-Humanoid-Direct-v0"
    seed: int = 401
    ...
    output_dir: str = "outputs/isaac_ppo_population"
    experiment_name: str = "humanoid_direct_fair_ppo_population"
    run_name_prefix: str = "agent"
    agent: IsaacLabPPOConfig = field(default_factory=IsaacLabPPOConfig)
...
def load_isaac_ppo_population_config(path: str | Path) -> IsaacLabPPOPopulationConfig:
    cfg = IsaacLabPPOPopulationConfig()
    data = load_yaml(path)
    if "agent" not in data:
        data["agent"] = {}
    data["agent"].setdefault("isaaclab_dir", data.get("isaaclab_dir", cfg.isaaclab_dir))
    data["agent"].setdefault("task", data.get("task", cfg.task))
    data["agent"].setdefault("experiment_name", data.get("experiment_name", cfg.experiment_name))
    cfg = _merge_dict_into_dataclass(cfg, data)
    cfg.agent.isaaclab_dir = cfg.isaaclab_dir
    cfg.agent.task = cfg.task
    cfg.agent.experiment_name = cfg.experiment_name
    cfg.agent.total_timesteps = per_agent_timesteps(
        cfg.total_timesteps,
...
def per_agent_timesteps(total_timesteps: int, num_train_agents: int, budget_mode: str) -> int:
