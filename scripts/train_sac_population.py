...
from humanoid_rl.config import SACActionAverageConfig, load_sac_action_average_config
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", type=str, default="configs/fair_sac_population.yaml")
    return parser.parse_args()


def make_agent_config(cfg: SACActionAverageConfig, agent_idx: int):
...
def main() -> None:
    args = parse_args()
    cfg = load_sac_action_average_config(args.config)
    output_dir = ensure_dir(cfg.output_dir)
    write_json(output_dir / "population_config.json", asdict(cfg))
    checkpoints: list[str] = []
    total_agent_steps = 0
    for agent_idx in tqdm(range(cfg.num_train_agents), desc="sac train agents", unit="agent"):
        agent_cfg = make_agent_config(cfg, agent_idx)
        trainer = SACTrainer(agent_cfg, run_name=f"sac_agent_{agent_idx}")
        trainer.dump_config()
        try:
            trainer.train(agent_cfg.total_timesteps)
